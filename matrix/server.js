 ...
 * Version: 4.1.007 - UI Improvements & Help Documentation
 * Run: node server.js
...
function handleDataRequest(req, res) {
    // Parse query params for action routing
    const reqUrl = req.url || '';
    const queryStr = reqUrl.split('?')[1] || '';
    const qparams = {};
    queryStr.split('&').forEach(pair => {
        const [k, v] = pair.split('=');
        if (k) qparams[k] = decodeURIComponent(v || '');
    });

    if (req.method === 'GET') {
        // Handle online tracker heartbeat
        if (qparams.action === 'online') {
            const onlineFile = path.join(dataDir, 'online_users.json');
            const ONLINE_TIMEOUT = 60; // seconds
            let users = {};
            try {
                if (fs.existsSync(onlineFile)) {
                    users = JSON.parse(fs.readFileSync(onlineFile, 'utf8'));
                }
            } catch (e) { users = {}; }
            ...
            for (const id of Object.keys(users)) {
                if ((now - users[id].lastSeen) > ONLINE_TIMEOUT) delete users[id];
            }
            // Register current user
            const onlineUserId = qparams.userId || 'unknown';
            users[onlineUserId] = { lastSeen: now, isEditor: qparams.isEditor === 'true' };
            try {
                fs.writeFileSync(onlineFile, JSON.stringify(users, null, 2));
            } catch (e) {
                if (DEBUG_MODE) console.error('Error writing online_users.json:', e.message);
            }

            let viewers = 0, editors = 0;
            for (const u of Object.values(users)) {
                u.isEditor ? editors++ : viewers++;
            }
        ...
        // Handle password verification (GET fallback, prefer POST)
        if (qparams.action === 'verify_password') {
            return sendJSON(res, 405, { error: 'Use POST for password verification' }, req);
        }
        ...
        // Handle password verification for clearAll
        if (qparams.action === 'verify_password') {
            const MAX_API_SIZE = 1048576; // 1MB
            let body = '';
            let tooBig = false;
            req.on('data', chunk => {
                body += chunk;
                if (body.length > MAX_API_SIZE) {
                    tooBig = true;
            ...
            req.on('end', async () => {
                ...
                try {
                    ...
                    const isValid = await bcrypt.compare(data.password || '', AUTH_PASSWORD_HASH);
                    return sendJSON(res, 200, { valid: isValid }, req);
                } catch (e) {
                    return sendJSON(res, 400, { error: 'Invalid request' }, req);
                }
            });
            return;
        }
...
function getLockStatus() {
    ...
    try {
        ...
        try { fs.unlinkSync(LOCK_FILE); } catch (_err) { /* ignore cleanup error */ }
        return { locked: false };
    }
}
    ...
    fs.readFile(filePath, (err, content) => {
        ...
        if (ext === '.html') {
            if (filePath.includes('draw-rooms') || filePath.includes('dev-tool')) {
                ...
                headers['Content-Security-Policy'] = "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:;";
            } else {
...
server.listen(PORT, HOST, () => {
    console.log('');
    console.log('╔════════════════════════════════════════════════════════════╗');
    console.log('║     TIESSE Matrix Network Server v4.1.007                  ║');
