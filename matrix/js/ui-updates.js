...
/**
 * Get device display name with prefix in purple HTML (like Devices table)
 * @param {Object} device - Device object
 * @returns {string} HTML with purple prefix + name
 */
function getDeviceDisplayNameHtml(device) {
    if (!device) return 'N/A';
    var prefix = device.prefix || getDefaultPrefix(device.type);
    var name = device.name || '';
    if (prefix) {
        // If name already starts with prefix, split it
        if (name.toUpperCase().indexOf(prefix.toUpperCase() + ' ') === 0) {
            var rawName = name.substring(prefix.length + 1);
            return '<span class="text-purple-600 font-bold">' + escapeHtml(prefix) + '</span> ' + escapeHtml(rawName);
        }
        return '<span class="text-purple-600 font-bold">' + escapeHtml(prefix) + '</span> ' + escapeHtml(name);
    }
...
function updateDevicesList() {
    ...
    } else if (appState.deviceView === 'grouped') {
        renderDevicesListGrouped(cont);
    } else {
...
function updateDevicesListCards(cont) {
    ...
    for (var i = 0; i < sorted.length; i++) {
        ...
        if (d.addresses && d.addresses.length > 0) {
            ipList = d.addresses.map(function(a) {
                var ip = a.network || a.ip || '';
                // Strip /mask suffix when separate mask field exists
                if (d.mask && ip.indexOf('/') !== -1) ip = ip.split('/')[0];
        ...
        var dhcpBadge = '';
        if (d.isDhcp) {
            dhcpBadge = '<span class="text-xs px-1.5 py-0.5 rounded-full bg-blue-100 text-blue-700 font-semibold">DHCP</span>';
        }
        
        // Monitoring badge (v4.0.000) — simple ON/OFF
        var monitorBadge = '';
        if (d.monitoringEnabled) {
            monitorBadge = '<span class="text-xs px-1.5 py-0.5 rounded-full bg-green-100 text-green-700 font-semibold">📡 ON</span>';
        }
        ...
        html += '<div class="border rounded-lg p-2 hover:shadow-md transition-shadow ' + opacityClass + ' ' + noConnectionsClass + '">' +
            ...
            '<a href="#" onclick="DeviceDetail.open(' + d.id + '); return false;" class="text-slate-400 hover:text-blue-600 text-sm p-1" title="View Details">🔍</a>' +
            '<span class="edit-mode-only flex flex-col gap-1">' +
            '<a href="#" onclick="addConnectionFromDevice(' + d.id + '); return false;" class="text-green-500 hover:text-green-700 text-sm p-1" title="Add Connection">➕</a>' +
            '<a href="#" onclick="copyDevice(' + d.id + '); return false;" class="text-purple-500 hover:text-purple-700 text-sm p-1" title="Duplicate Device">📋</a>' +
            '<a href="#" onclick="editDevice(' + d.id + '); return false;" class="text-blue-500 hover:text-blue-700 text-sm p-1" title="Edit Device">✎</a>' +
            '<a href="#" onclick="removeDevice(' + d.id + '); return false;" class="text-red-500 hover:text-red-700 text-sm p-1" title="Delete Device">✕</a>' +
            '</span>' +
            '</div>' +
            '</div>' +
            '</div>';
...
function updateDevicesListTable(cont) {
    ...
    var sortIcon = function(key) {
        if (appState.deviceSort.key === key) {
            return '<span class="ml-1 opacity-80">' + (appState.deviceSort.asc ? '▲' : '▼') + '</span>';
        }
        return '<span class="ml-1 opacity-50">↕</span>';
    };

    var html = '<table class="w-full text-xs border-collapse">';
    html += '<thead><tr class="bg-slate-600 text-white">';
    html += '<th class="p-2 text-left cursor-pointer hover:bg-slate-500 whitespace-nowrap uppercase" onclick="toggleDeviceSort(\'location\')">Location ' + sortIcon('location') + '</th>';
    html += '<th class="p-2 text-left cursor-pointer hover:bg-slate-500 whitespace-nowrap uppercase" onclick="toggleDeviceSort(\'rack\')">Group ' + sortIcon('rack') + '</th>';
    html += '<th class="p-2 text-center cursor-pointer hover:bg-slate-500 whitespace-nowrap uppercase" onclick="toggleDeviceSort(\'order\')">Order/Rack Pos/ID# ' + sortIcon('order') + '</th>';
    html += '<th class="p-2 text-left cursor-pointer hover:bg-slate-500 whitespace-nowrap uppercase" onclick="toggleDeviceSort(\'name\')">Device ' + sortIcon('name') + '</th>';
    html += '<th class="p-2 text-left cursor-pointer hover:bg-slate-500 whitespace-nowrap uppercase" onclick="toggleDeviceSort(\'brandModel\')">Brand/Model ' + sortIcon('brandModel') + '</th>';
    html += '<th class="p-2 text-left cursor-pointer hover:bg-slate-500 whitespace-nowrap uppercase" onclick="toggleDeviceSort(\'type\')">Type ' + sortIcon('type') + '</th>';
    html += '<th class="p-2 text-center cursor-pointer hover:bg-slate-500 whitespace-nowrap uppercase" onclick="toggleDeviceSort(\'status\')">Status ' + sortIcon('status') + '</th>';
    html += '<th class="p-2 text-left whitespace-nowrap uppercase">IP/Network</th>';
    html += '<th class="p-2 text-left whitespace-nowrap uppercase">Service</th>';
    html += '<th class="p-2 text-center cursor-pointer hover:bg-slate-500 whitespace-nowrap uppercase" onclick="toggleDeviceSort(\'ports\')">Ports ' + sortIcon('ports') + '</th>';
    html += '<th class="p-2 text-center cursor-pointer hover:bg-slate-500 whitespace-nowrap uppercase" onclick="toggleDeviceSort(\'connections\')">Conn ' + sortIcon('connections') + '</th>';
    ...
    for (var i = 0; i < sorted.length; i++) {
        ...
        // DHCP/Monitoring badges for table (v4.0.000)
        if (d.isDhcp) statusBadge += ' <span class="px-1 py-0.5 text-[9px] rounded bg-blue-100 text-blue-700 font-bold">DHCP</span>';
        if (d.monitoringEnabled) statusBadge += ' <span class="px-1 py-0.5 text-[9px] rounded bg-green-100 text-green-700 font-bold">📡</span>';
        ...
        // Extract prefix/sigla for violet display
        var devicePrefix = d.prefix || getDefaultPrefix(d.type);
        var rawName = escapeHtml(d.name || '');
        ...
        html += '<td class="p-2 font-semibold text-slate-800">' + (disabled ? '<span class="text-red-500" title="Disabled">✗</span> ' : '') + (devicePrefix ? '<span class="text-purple-600 font-bold">' + escapeHtml(devicePrefix) + '</span> ' : '') + rawName + '</td>';
        ...
        html += '<a href="#" onclick="DeviceDetail.open(' + d.id + '); return false;" class="text-blue-500 hover:text-blue-700 text-xs mr-1" title="View Details">🔍</a>';
        html += '<span class="edit-mode-only">';
        html += '<a href="#" onclick="addConnectionFromDevice(' + d.id + '); return false;" class="text-green-600 hover:text-green-900 text-xs mr-1" title="Add Connection">+Conn</a>';
        html += '<a href="#" onclick="copyDevice(' + d.id + '); return false;" class="text-purple-600 hover:text-purple-900 text-xs mr-1" title="Duplicate Device">Copy</a>';
        html += '<a href="#" onclick="editDevice(' + d.id + '); return false;" class="text-blue-600 hover:text-blue-900 text-xs mr-1">Edit</a>';
        html += '<a href="#" onclick="removeDevice(' + d.id + '); return false;" class="text-red-600 hover:text-red-900 text-xs">Del</a>';
        html += '</span>';
        html += '</td>';
        html += '</tr>';
    }
...
var SVGMatrix = (function() {
    ...
    function render() {
        ...
        for (var i = 0; i < sorted.length; i++) {
            ...
            html += '<div style="width:' + cellSize + 'px;min-width:' + cellSize + 'px;height:100%;background:' + colors.headerBg + ';border-left:3px solid ' + rackColor + ';display:flex;flex-direction:column;align-items:center;justify-content:center;padding:4px;box-sizing:border-box;" title="' + escapeXml(getDeviceDisplayName(d)) + '">' +
        ...
        for (var r = 0; r < sorted.length; r++) {
            ...
            html += '<div style="width:100%;height:' + cellSize + 'px;background:' + rowBg + ';border-left:3px solid ' + rowRackColor + ';border-bottom:1px solid ' + colors.border + ';display:flex;flex-direction:column;align-items:center;justify-content:center;padding:4px;box-sizing:border-box;" title="' + escapeXml(getDeviceDisplayName(row)) + '">' +
                ...
                '<div style="font-size:9px;font-weight:bold;color:' + (rowDisabled ? 'var(--color-text-light)' : 'var(--color-text)') + ';overflow:hidden;text-overflow:ellipsis;white-space:nowrap;width:100%;text-align:center;' + (rowDisabled ? 'text-decoration:line-through;' : '') + '">' + rowPosNum + '-' + escapeXml(getDeviceDisplayName(row)) + '</div>' +
    ...
    function exportPNG() {
        ...
        for (var i = 0; i < sorted.length; i++) {
            ...
            var nameText = createSvgText(x + cellSize/2, 48, posNum + '-' + (getDeviceDisplayName(d) || '').substring(0,15), textColor, '9', {fontWeight: 'bold'});
        ...
        for (var r = 0; r < sorted.length; r++) {
            ...
            var rowNameText = createSvgText(headerWidth/2, y + 48, rowPosNum + '-' + (getDeviceDisplayName(row) || '').substring(0,15), rowTextColor, '9', {fontWeight: 'bold'});
...
function showMatrixTooltip(event, connIdx) {
    ...
    if (fromDevice) {
        html += '<div class="font-bold text-white text-base">' + getDeviceDisplayName(fromDevice) + '</div>';
    ...
    if (toDevice) {
        html += '<div class="font-bold text-white text-base">' + getDeviceDisplayName(toDevice) + '</div>';
        html += '<div class="text-sm text-slate-400 mt-2 leading-relaxed space-y-1">';
...
function updateConnFilterBar() {
    ...
    for (var i = 0; i < appState.devices.length; i++) {
        ...
        // Cascade: only show groups that belong to the selected location
        if (d.rackId && sources.indexOf(d.rackId) === -1) {
            if (!selectedLocation || d.location === selectedLocation) {
                sources.push(d.rackId);
            }
...
function updateConnectionsList() {
    ...
    // Render connections table based on view mode
    if (appState.connViewMode === 'grouped') {
        renderConnectionsTableGrouped(cont);
    } else {
...
function updateConnectionsListOnly() {
    ...
    // Render connections table without touching filter bar based on view mode
    if (appState.connViewMode === 'grouped') {
        renderConnectionsTableGrouped(cont);
    } else {
        renderConnectionsTable(cont);
...
function renderConnectionsTable(cont) {
    ...
    html += '<div class="overflow-x-auto"><table class="min-w-full divide-y text-xs"><thead class="bg-slate-600 text-white"><tr>';
        ...
        if (hdr.key !== 'actions' && hdr.key !== 'arrow') {
            ...
            for (var si = 0; si < appState.connSort.length; si++) {
                if (appState.connSort[si].key === hdr.key) {
                    ...
                    sortIndicator = '<span class="ml-1 opacity-80">' + arrowSort + level + '</span>';
                    isSorted = true;
                    break;
                }
            }
            // Show default sort icon if not currently sorted
            if (!isSorted) {
                sortIndicator = '<span class="ml-1 opacity-50">↕</span>';
            }
        }
        
        if (hdr.key === 'actions') {
            html += '<th class="px-3 py-2 text-left text-xs font-medium uppercase no-print edit-mode-only whitespace-nowrap">' + hdr.label + '</th>';
        } else if (hdr.key === 'arrow') {
            html += '<th class="px-3 py-2 text-left text-xs font-medium uppercase whitespace-nowrap' + printClass + '">' + hdr.label + '</th>';
        } else {
            html += '<th onclick="toggleConnSort(\'' + hdr.key + '\', event)" class="px-3 py-2 text-left text-xs font-medium uppercase cursor-pointer hover:bg-slate-500 whitespace-nowrap' + printClass + '" title="Click to sort, Shift+Click to add level">' + hdr.label + ' ' + sortIndicator + '</th>';
        }
    }
    ...
    sorted.sort(function(a, b) {
        ...
        function valueFor(item, key) {
            ...
            switch (key) {
                ...
                case 'fromDevice': return fromDevice ? (getDeviceDisplayName(fromDevice) || '') : '';
                case 'fromPort': return fromPort || '';
                case 'toPort': return toPort || '';
                case 'toDevice': return toDevice ? (getDeviceDisplayName(toDevice) || '') : (c.externalDest || '');
    ...
    for (var i = 0; i < sorted.length; i++) {
        ...
        if (toDevice) {
            toDisplayName = escapeHtml(getDeviceDisplayName(toDevice));
            toDisplayRack = escapeHtml(toDevice.rackId);
            toDisplayPos = String(toDevice.order).padStart(2, '0');
        } else if (c.isWallJack) {
            var wjTypeLabel = (c.wallJackType === 'rj11') ? ' (RJ11)' : ' (RJ45)';
            toDisplayName = '🔌 ' + escapeHtml(c.externalDest || 'Wall Jack') + wjTypeLabel;
        ...
        var fromDeviceNameEscaped = fromDevice ? escapeHtml(getDeviceDisplayName(fromDevice)).replace(/'/g, "\\'") : '';
        var toDeviceNameEscaped = toDevice ? escapeHtml(getDeviceDisplayName(toDevice)).replace(/'/g, "\\'") : '';
        ...
        html += '<tr class="' + rowBg + ' ' + opacityClass + ' hover:bg-blue-50 ' + borderClass + '"' + rowTitle + '>' +
            ...
            '<td class="px-3 py-2 align-top">' +
                '<div class="font-semibold cursor-pointer hover:text-blue-600" onclick="filterConnectionsByDevice(\'' + fromDeviceNameEscaped + '\')">' + fromDisabledIndicator + (fromDevice ? getDeviceDisplayNameHtml(fromDevice) : 'N/A') + '</div>' +
            ...
            '<td class="px-3 py-2 align-top">' +
                '<div class="font-semibold cursor-pointer hover:text-blue-600" onclick="filterConnectionsByDevice(\'' + toDeviceNameEscaped + '\')">' + toDisabledIndicator + (toDevice ? getDeviceDisplayNameHtml(toDevice) : toDisplayName) + '</div>' +
            ...
            '<td class="px-3 py-2 align-top text-center whitespace-nowrap no-print edit-mode-only">' +
            '<a href="#" onclick="editConnection(' + origIdx + '); return false;" class="text-blue-600 hover:text-blue-900 hover:underline text-xs mr-2">Edit</a>' +
            '<a href="#" onclick="removeConnection(' + origIdx + '); return false;" class="text-red-600 hover:text-red-900 hover:underline text-xs">Del</a>' +
            '</td>' +
            '</tr>';
    }
...
function exportExcel() {
    try {
        ...
        var infoData = [
            ...
            { 'Field': 'Locations', 'Value': (appState.locations || []).length },
            { 'Field': 'Rooms', 'Value': (appState.rooms || []).length },
            { 'Field': 'Groups', 'Value': (appState.groups || []).length }
        ];
        ...
        for (var i = 0; i < appState.devices.length; i++) {
            ...
            // Build port summary (e.g. "8xeth, 2xSFP/SFP+, 1xWAN")
            var portSummary = '';
            if (d.ports && d.ports.length > 0) {
                var ptCounts = {};
                d.ports.forEach(function(p) { var t = p.type || 'unknown'; ptCounts[t] = (ptCounts[t] || 0) + 1; });
                portSummary = Object.keys(ptCounts).map(function(t) { return ptCounts[t] + 'x' + t; }).join(', ');
            }
            devData.push({
                'Location': d.location || '',
        ...
        for (var j = 0; j < appState.connections.length; j++) {
            ...
            connData.push({
                ...
                'Src Device': fromDevice ? getDeviceDisplayName(fromDevice) : '',
                'Src Port': c.fromPort,
                'Dst Port': c.toPort,
                'Dst Device': toDevice ? getDeviceDisplayName(toDevice) : (c.isWallJack ? '[WJ ' + (c.wallJackType || 'rj45').toUpperCase() + '] ' + c.externalDest : (c.externalDest ? '[EXT] ' + c.externalDest : '')),
                'Dst Pos': toDevice ? toDevice.order : '',
                'Dst Rack': toDevice ? toDevice.rackId : (c.isWallJack ? 'Wall Jack' : (c.externalDest ? 'ISP' : '')),
                'WJ Type': c.isWallJack ? (c.wallJackType || 'rj45').toUpperCase() : '',
        ...
        if (sorted.length > 0) {
            ...
            for (var m = 0; m < sorted.length; m++) {
                var dev = sorted[m];
                matrixHeader.push('[' + dev.rackId + '-' + String(dev.order).padStart(2, '0') + '] ' + getDeviceDisplayName(dev));
            }
            var matrixData = [matrixHeader];

            for (var r = 0; r < sorted.length; r++) {
                var row = sorted[r];
                var rowData = ['[' + row.rackId + '-' + String(row.order).padStart(2, '0') + '] ' + getDeviceDisplayName(row)];
        ...
        if (appState.locations && appState.locations.length > 0) {
            ...
            for (var li = 0; li < appState.locations.length; li++) {
                ...
                var locConnCount = appState.connections.filter(function(c) {
                    var fromDev = appState.devices.find(function(d) { return d.id === c.from; });
                    return fromDev && fromDev.location === loc.name;
                }).length;
                var locGroupCount = (appState.groups || []).filter(function(g) { return g.location === loc.name || g.location === loc.id; }).length;
                locsData.push({
                    'Code': loc.code,
                    'Name': loc.name,
                    'Type': loc.type,
                    ...
                    'Connections': locConnCount,
                    'Groups': locGroupCount
                });
            }
            XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(locsData), 'Locations');
        }

        // Add Groups sheet (v4.0.000)
        if (appState.groups && appState.groups.length > 0) {
            var groupsData = [];
            for (var gi = 0; gi < appState.groups.length; gi++) {
                var grp = appState.groups[gi];
                var grpDevCount = appState.devices.filter(function(d) { return d.rackId === grp.code; }).length;
                groupsData.push({
                    'Code': grp.code,
                    'Name': grp.name,
                    'Type': grp.type || '',
            ...
            XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(groupsData), 'Groups');
        }
...
// ============================================================================
// CONNECTIONS TABLE - GROUPED VIEW (by device source)
// ============================================================================
/**
 * Render connections table grouped by source device
 * Shows each device ONCE with all its ports and connections in an expandable section
 * This solves the issue where the same device appears many times with different connections
 */
function renderConnectionsTableGrouped(cont) {
    if (!cont) return;
    ...
    // Group connections by FROM device
    var deviceGroups = {};
    var deviceOrder = [];
    ...
    for (var i = 0; i < filteredItems.length; i++) {
        ...
        var c = item._original;
        var fromId = item._isMirrored ? item.from : c.from;
        var fromDevice = null;

        for (var k = 0; k < appState.devices.length; k++) {
            if (appState.devices[k].id === fromId) {
                fromDevice = appState.devices[k];
                break;
            }
        }
        ...
        var deviceKey = fromId + '-' + (fromDevice.rackId || 'unknown');
        ...
        deviceGroups[deviceKey].connections.push(item);

        var portKey = item._isMirrored ? item.fromPort : c.fromPort;
        if (!deviceGroups[deviceKey].ports[portKey]) {
            deviceGroups[deviceKey].ports[portKey] = [];
        }
        deviceGroups[deviceKey].ports[portKey].push(item);
    }

    // Sort device groups: prioritize filtered group first, then alphabetical
    var activeSourceFilter = (appState.connFilters && appState.connFilters.source) ? appState.connFilters.source.toLowerCase() : '';
    deviceOrder.sort(function(a, b) {
        var devA = deviceGroups[a].device;
        var devB = deviceGroups[b].device;
        // If a source/group filter is active, matching devices come first
        if (activeSourceFilter) {
            var aMatch = (devA.rackId || '').toLowerCase() === activeSourceFilter ? 0 : 1;
            var bMatch = (devB.rackId || '').toLowerCase() === activeSourceFilter ? 0 : 1;
            if (aMatch !== bMatch) return aMatch - bMatch;
        }
        var nameA = (getDeviceDisplayName(devA) || '').toLowerCase();
        var nameB = (getDeviceDisplayName(devB) || '').toLowerCase();
        return nameA < nameB ? -1 : nameA > nameB ? 1 : 0;
    });

    var totalGroups = deviceOrder.length;
    var totalConns = filteredItems.length;

    var html = '<div class="space-y-2">';
    html += '<div class="flex items-center justify-between px-1 mb-1"><span class="text-[10px] text-slate-400 font-medium">' + totalGroups + ' groups · ' + totalConns + ' connections</span></div>';

    for (var dg = 0; dg < deviceOrder.length; dg++) {
        var key = deviceOrder[dg];
        var group = deviceGroups[key];
        var device = group.device;
        var groupId = 'conn-group-' + device.id;
        var isExpanded = localStorage.getItem(groupId + '-expanded') !== 'false';

        var connCount = group.connections.length;
        var portCount = Object.keys(group.ports).length;
        var statusOpacity = device.status === 'disabled' ? ' opacity-50' : '';
        var rackColor = getRackColor(device.rackId);
        var rearTag = (device.isRear || device.rear) ? ' <span class="text-amber-500 text-xs font-bold">↩</span>' : '';
        var locationLabel = (typeof formatLocationLabel === 'function') ? (formatLocationLabel(device) || '-') : '-';

        // Group container with left accent bar
        html += '<div class="rounded-lg overflow-hidden' + statusOpacity + '" style="border:1px solid #e2e8f0;border-left:4px solid ' + rackColor + ';">';

        // Header - subtle light gradient using rack color
        var devPos = String(device.order || 0).padStart(2, '0');
        html += '<button onclick="toggleConnGroup(\'' + groupId + '\')" class="w-full px-3 py-1.5 text-left flex items-center justify-between transition-colors" style="background:linear-gradient(to right,' + rackColor + '08,#f8fafc);">';
        html += '<div class="flex items-center gap-2">';
        html += '<span class="transition-transform font-bold text-[10px] text-slate-400" id="' + groupId + '-arrow" style="transform:rotate(' + (isExpanded ? 0 : -90) + 'deg);">▼</span>';
        html += '<span class="px-1.5 py-0.5 rounded text-xs font-bold uppercase" style="background:' + rackColor + '18;color:' + rackColor + ';border:1px solid ' + rackColor + '40;">' + escapeHtml(device.rackId || '-') + '</span>';
        html += '<span class="text-xs font-bold text-slate-700">' + getDeviceDisplayNameHtml(device) + '</span>' + rearTag;
        html += '<span class="text-[10px] text-slate-400 ml-1">Position / ID# <span class="text-xs font-bold text-slate-600">' + devPos + '</span></span>';
        html += '</div>';
        html += '<div class="flex items-center gap-2">';
        html += '<span class="text-[10px] text-slate-400">📍 ' + escapeHtml(locationLabel) + '</span>';
        html += '<span class="px-2 py-0.5 rounded text-[10px] font-semibold" style="background:' + rackColor + '15;color:' + rackColor + ';border:1px solid ' + rackColor + '30;">' + connCount + ' conn · ' + portCount + '/' + (device.ports ? device.ports.length : 0) + ' ports</span>';
        html += '</div>';
        html += '</button>';

        // Content
        html += '<div id="' + groupId + '-content" class="' + (isExpanded ? '' : 'hidden ') + '">';
        html += '<table class="w-full text-xs border-collapse" style="table-layout:fixed">';
        html += '<colgroup>';
        html += '<col style="width:9%">';    // Src Port
        html += '<col style="width:3%">';    // ⟷
        html += '<col style="width:9%">';    // Dst Port
        html += '<col style="width:20%">';   // To Device
        html += '<col style="width:4%">';    // Pos
        html += '<col style="width:14%">';   // Destination
        html += '<col style="width:10%">';   // Type
        html += '<col style="width:6%">';    // Cable
        html += '<col style="width:6%">';    // Status
        html += '<col style="width:13%">';   // Notes
        html += '<col style="width:6%">';    // Actions
        html += '</colgroup>';
        // Column headers
        html += '<thead><tr class="bg-slate-500 text-white">';
        html += '<th class="px-2 py-1.5 text-center uppercase text-[10px] font-semibold">Src Port</th>';
        html += '<th class="px-2 py-1.5 text-center uppercase text-[10px] font-semibold"></th>';
        html += '<th class="px-2 py-1.5 text-center uppercase text-[10px] font-semibold">Dst Port</th>';
        html += '<th class="px-2 py-1.5 text-left uppercase text-[10px] font-semibold">To Device</th>';
        html += '<th class="px-2 py-1.5 text-center uppercase text-[10px] font-semibold">Position / ID#</th>';
        html += '<th class="px-2 py-1.5 text-left uppercase text-[10px] font-semibold">Destination</th>';
        html += '<th class="px-2 py-1.5 text-center uppercase text-[10px] font-semibold">Type</th>';
        html += '<th class="px-2 py-1.5 text-center uppercase text-[10px] font-semibold">Cable</th>';
        html += '<th class="px-2 py-1.5 text-center uppercase text-[10px] font-semibold">Status</th>';
        html += '<th class="px-2 py-1.5 text-left uppercase text-[10px] font-semibold">Notes</th>';
        html += '<th class="px-2 py-1.5 text-center uppercase text-[10px] font-semibold no-print edit-mode-only">Actions</th>';
        html += '</tr></thead><tbody>';

        var portKeys = Object.keys(group.ports).sort();
        var rowIdx = 0;
        for (var p = 0; p < portKeys.length; p++) {
            var portName = portKeys[p];
            var portConns = group.ports[portName];

            for (var pc = 0; pc < portConns.length; pc++) {
                var item = portConns[pc];
                var c = item._original;
                var isMirrored = item._isMirrored;
                ...
                var toDevice = null;
                for (var td = 0; td < appState.devices.length; td++) {
                    if (appState.devices[td].id === toId) {
                        toDevice = appState.devices[td];
                        break;
                    }
                }

                // To device info (same logic as Connections Table view)
                var toDisplayName, toDisplayRack, toDisplayPos;
                if (toDevice) {
                    toDisplayName = escapeHtml(getDeviceDisplayName(toDevice));
                    toDisplayRack = escapeHtml(toDevice.rackId || '-');
                    toDisplayPos = String(toDevice.order || 0).padStart(2, '0');
                } else if (c.isWallJack) {
                    var wjTypeLabel2 = (c.wallJackType === 'rj11') ? ' (RJ11)' : ' (RJ45)';
                    toDisplayName = '🔌 ' + escapeHtml(c.externalDest || 'Wall Jack') + wjTypeLabel2;
                    toDisplayRack = 'Wall Jack';
                    toDisplayPos = '-';
                } else if (c.externalDest) {
                    toDisplayName = '📡 ' + escapeHtml(c.externalDest);
                ...
                // To device IPs (same logic as Connections Table view)
                var toIPs = '';
                if (toDevice && toDevice.addresses && toDevice.addresses.length > 0) {
                    var addrs2 = [];
                    for (var ai2 = 0; ai2 < toDevice.addresses.length; ai2++) {
                ...
                var toRearIndicator = (toDevice && (toDevice.isRear || toDevice.rear))
                    ? '<span class="text-[10px] text-amber-600 font-bold" title="Rear">↩</span>'
                    : '<span class="text-[10px] opacity-0">↩</span>';

                var toRackColor = toDevice ? getRackColor(toDevice.rackId) : (c.isWallJack ? (config.connColors ? config.connColors['wallport'] : '#ec4899') : (c.externalDest ? '#ef4444' : '#94a3b8'));
                var toDisabledIndicator = (toDevice && toDevice.status === 'disabled') ? '<span class="text-red-500 mr-0.5" title="Disabled">✗</span>' : '';

                var connColor = (config.connColors ? config.connColors[c.type] : null) || '#94a3b8';
                var typeLabel = config.connLabels[c.type] || c.type || 'Unknown';
                var statusClass = (c.status === 'disabled') ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800';
                var statusText = (c.status === 'disabled') ? 'Off' : 'On';
                var markerHtml = createMarkerHtml(c.cableMarker, c.cableColor, false);

                var rowBg = c.flagged ? 'bg-red-200' : (isMirrored ? 'bg-red-100' : ((rowIdx % 2 === 0) ? '' : 'bg-slate-50'));
                var mirroredClass = isMirrored ? ' italic opacity-70' : '';
                var disabledClass = (c.status === 'disabled') ? ' opacity-50' : '';
                var borderClass = isMirrored ? 'border-b border-red-300' : 'border-b border-slate-100';

                html += '<tr class="' + rowBg + mirroredClass + disabledClass + ' hover:bg-blue-50 ' + borderClass + ' text-slate-700">';
                html += '<td class="px-2 py-1.5 font-mono font-semibold text-center truncate">' + escapeHtml(portName) + (isMirrored ? ' <span class="text-red-600 font-bold" title="Mirrored">⇄</span>' : '') + '</td>';
                html += '<td class="px-1 py-1.5 text-center"><span style="font-size:16px;font-weight:bold;color:' + (isMirrored ? 'var(--color-danger-hover)' : 'var(--color-primary-hover)') + ';">⟷</span></td>';
                html += '<td class="px-2 py-1.5 font-mono text-center text-slate-600 truncate">' + escapeHtml(displayToPort || '-') + '</td>';
                html += '<td class="px-2 py-1.5" style="overflow:hidden;text-overflow:ellipsis"><div class="font-semibold truncate">' + toDisabledIndicator + (toDevice ? getDeviceDisplayNameHtml(toDevice) : toDisplayName) + '</div>' +
                    (toIPs ? '<div class="text-[10px] text-slate-500 font-mono truncate">' + toIPs + '</div>' : '') + '</td>';
                html += '<td class="px-2 py-1.5 text-center"><span class="inline-flex items-center justify-center gap-0.5"><span class="px-1.5 py-0.5 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">' + toDisplayPos + '</span>' + toRearIndicator + '</span></td>';
                html += '<td class="px-2 py-1.5 font-bold truncate" style="color:' + toRackColor + '">' + toDisplayRack + '</td>';
                html += '<td class="px-2 py-1.5 text-center"><span class="px-1.5 py-0.5 text-[10px] font-semibold rounded-full text-white" style="background-color:' + connColor + '">' + escapeHtml(typeLabel) + '</span></td>';
                html += '<td class="px-2 py-1.5 text-center">' + markerHtml + '</td>';
                html += '<td class="px-2 py-1.5 text-center"><span class="px-1.5 py-0.5 text-[10px] font-semibold rounded-full ' + statusClass + '">' + statusText + '</span></td>';
                html += '<td class="px-2 py-1.5 text-[10px] italic text-slate-500 truncate">' + escapeHtml(c.notes || '') + '</td>';
                html += '<td class="px-2 py-1.5 text-center whitespace-nowrap no-print edit-mode-only">';
                html += '<a href="#" onclick="editConnection(' + item._originalIndex + '); return false;" class="text-blue-600 hover:text-blue-900 hover:underline text-xs mr-2">Edit</a>';
                html += '<a href="#" onclick="removeConnection(' + item._originalIndex + '); return false;" class="text-red-600 hover:text-red-900 hover:underline text-xs">Del</a>';
                html += '</td>';
                html += '</tr>';
                rowIdx++;
            }
...
/**
 * Toggle expansion of a connection group
 */
function toggleConnGroup(groupId) {
    ...
    if (content) {
        ...
        if (isHidden) {
            ...
            localStorage.setItem(groupId + '-expanded', 'true');
            if (arrow) arrow.style.transform = 'rotate(0deg)';
        } else {
            content.classList.add('hidden');
            localStorage.setItem(groupId + '-expanded', 'false');
            if (arrow) arrow.style.transform = 'rotate(-90deg)';
        }
    }
}

function expandAllConnGroups() {
    for (var i = 0; i < appState.devices.length; i++) {
        var dev = appState.devices[i];
        var groupId = 'conn-group-' + dev.id;
        var content = document.getElementById(groupId + '-content');
        var arrow = document.getElementById(groupId + '-arrow');
...
function collapseAllConnGroups() {
    for (var i = 0; i < appState.devices.length; i++) {
        var dev = appState.devices[i];
        var groupId = 'conn-group-' + dev.id;
        var content = document.getElementById(groupId + '-content');
...
function expandAllDeviceGroups() {
    ...
    for (var i = 0; i < elements.length; i++) {
        ...
        var groupId = content.id.replace('-content', '');
        var arrow = document.getElementById(groupId + '-arrow');
        content.classList.remove('hidden');
        localStorage.setItem(groupId + '-expanded', 'true');
        if (arrow) arrow.style.transform = 'rotate(0deg)';
...
// ============================================================================
// DEVICES LIST - GROUPED VIEW
// ============================================================================
/**
 * Render devices grouped by rackId (Group)
 * Each group is collapsible with expand/collapse toggle
 * Unified color scheme: slate-700 headers + rackColor accent bar
 * Same columns as Table view for consistency
 */
function renderDevicesListGrouped(cont) {
    if (!cont) return;

    var filtered = typeof getFilteredDevices === 'function' ? getFilteredDevices() : appState.devices;

    if (filtered.length === 0) {
        cont.innerHTML = '<p class="text-slate-400 text-center py-6 text-sm">No devices match the current filters</p>';
        return;
    }

    // Group devices by rackId
    var groups = {};
    var groupOrder = [];
    for (var i = 0; i < filtered.length; i++) {
        var d = filtered[i];
        var groupKey = d.rackId || 'Ungrouped';
        if (!groups[groupKey]) {
            groups[groupKey] = [];
    ...
    var totalGroups = groupOrder.length;
    var totalDevices = filtered.length;

    var html = '<div class="space-y-2">';
    html += '<div class="flex items-center justify-between px-1 mb-1"><span class="text-[10px] text-slate-400 font-medium">' + totalGroups + ' groups · ' + totalDevices + ' devices</span></div>';

    for (var g = 0; g < groupOrder.length; g++) {
        var key = groupOrder[g];
        var devices = groups[key];
        var groupId = 'dev-group-' + key.replace(/[^a-zA-Z0-9]/g, '_');
        var isExpanded = localStorage.getItem(groupId + '-expanded') !== 'false';
        var rackColor = getRackColor(key);

        // Group container with left accent bar
        html += '<div class="rounded-lg overflow-hidden" style="border:1px solid #e2e8f0;border-left:4px solid ' + rackColor + ';">';

        // Header - subtle light gradient
        html += '<button onclick="toggleDeviceGroup(\'' + groupId + '\')" class="w-full px-3 py-1.5 text-left flex items-center justify-between transition-colors" style="background:linear-gradient(to right,' + rackColor + '08,#f8fafc);">';
        html += '<div class="flex items-center gap-2">';
        html += '<span class="transition-transform font-bold text-[10px] text-slate-400" id="' + groupId + '-arrow" style="transform:rotate(' + (isExpanded ? 0 : -90) + 'deg);">▼</span>';
        html += '<span class="px-1.5 py-0.5 rounded text-xs font-bold uppercase" style="background:' + rackColor + '18;color:' + rackColor + ';border:1px solid ' + rackColor + '40;">' + escapeHtml(key) + '</span>';
        html += '</div>';
        html += '<span class="px-2 py-0.5 rounded text-[10px] font-semibold" style="background:' + rackColor + '15;color:' + rackColor + ';border:1px solid ' + rackColor + '30;">' + devices.length + ' devices</span>';
        html += '</button>';
        ...
        html += '<table class="w-full text-xs border-collapse" style="table-layout:fixed">';
        html += '<colgroup>';
        html += '<col style="width:11%">';   // Location
        html += '<col style="width:4%">';    // Pos
        html += '<col style="width:13%">';   // Device
        html += '<col style="width:10%">';   // Brand/Model
        html += '<col style="width:8%">';    // Type
        html += '<col style="width:8%">';    // Status
        html += '<col style="width:13%">';   // IP/Network
        html += '<col style="width:9%">';    // Service
        html += '<col style="width:4%">';    // Ports
        html += '<col style="width:4%">';    // Conn
        html += '<col style="width:16%">';   // Actions
        html += '</colgroup>';
        // Column headers - removed redundant Group column (already shown in group header)
        html += '<thead><tr class="bg-slate-500 text-white">';
        html += '<th class="px-2 py-1.5 text-left uppercase text-[10px] font-semibold">Location</th>';
        html += '<th class="px-2 py-1.5 text-center uppercase text-[10px] font-semibold" style="width:55px">Pos</th>';
        html += '<th class="px-2 py-1.5 text-left uppercase text-[10px] font-semibold">Device</th>';
        html += '<th class="px-2 py-1.5 text-left uppercase text-[10px] font-semibold">Brand/Model</th>';
        html += '<th class="px-2 py-1.5 text-left uppercase text-[10px] font-semibold">Type</th>';
        html += '<th class="px-2 py-1.5 text-center uppercase text-[10px] font-semibold">Status</th>';
        html += '<th class="px-2 py-1.5 text-left uppercase text-[10px] font-semibold">IP/Network</th>';
        html += '<th class="px-2 py-1.5 text-left uppercase text-[10px] font-semibold">Service</th>';
        html += '<th class="px-2 py-1.5 text-center uppercase text-[10px] font-semibold" style="width:50px">Ports</th>';
        html += '<th class="px-2 py-1.5 text-center uppercase text-[10px] font-semibold" style="width:40px">Conn</th>';
        html += '<th class="px-2 py-1.5 text-center uppercase text-[10px] font-semibold no-print edit-mode-only" style="width:70px">Actions</th>';
        html += '</tr></thead><tbody>';

        devices.sort(function(a, b) { return (a.order || 0) - (b.order || 0); });

        for (var di = 0; di < devices.length; di++) {
            var dev = devices[di];
            var disabled = dev.status === 'disabled';

            // Used ports count
            var usedPorts = 0;
            if (dev.ports) {
                for (var pj = 0; pj < dev.ports.length; pj++) {
                    if (typeof isPortUsed === 'function' && isPortUsed(dev.id, dev.ports[pj].name)) usedPorts++;
                }
            }
            ...
            var totalConnections = 0;
            for (var ci = 0; ci < appState.connections.length; ci++) {
                if (appState.connections[ci].from === dev.id || appState.connections[ci].to === dev.id) totalConnections++;
            }

            var isWirelessDevice = ['wifi', 'router_wifi', 'access_point'].indexOf(dev.type) !== -1;
            var statusBadge = disabled
                ? '<span class="px-1.5 py-0.5 text-xs rounded-full bg-red-100 text-red-800">OFF</span>'
                : '<span class="px-1.5 py-0.5 text-xs rounded-full bg-green-100 text-green-800">ON</span>';
            if (dev.isDhcp) statusBadge += ' <span class="px-1 py-0.5 text-[9px] rounded bg-blue-100 text-blue-700 font-bold">DHCP</span>';
            if (dev.monitoringEnabled) statusBadge += ' <span class="px-1 py-0.5 text-[9px] rounded bg-green-100 text-green-700 font-bold">📡</span>';

            // IP/Network
            var addressText = '';
            var ipList = [];
            if (dev.addresses && dev.addresses.length > 0) {
                ipList = dev.addresses.map(function(a) {
                    var ip = a.network || a.ip || '';
                    if (dev.mask && ip.indexOf('/') !== -1) ip = ip.split('/')[0];
                    return ip;
                }).filter(Boolean);
            } else {
                ipList = [dev.ip1, dev.ip2, dev.ip3, dev.ip4].filter(function(ip) { return ip && ip.trim(); });
            }
            if (ipList.length > 0) addressText = ipList.join(', ');

            var rowBg = (di % 2 === 0) ? 'bg-white' : 'bg-slate-50';
            var warningClass = (totalConnections === 0 && !isWirelessDevice) ? 'bg-orange-50' : (totalConnections === 0 && isWirelessDevice) ? 'bg-cyan-50' : rowBg;

            var locationLabel = (typeof formatLocationLabel === 'function') ? (formatLocationLabel(dev) || '-') : '-';
            var devicePrefix = dev.prefix || (typeof getDefaultPrefix === 'function' ? getDefaultPrefix(dev.type) : '');
            var rawName = escapeHtml(dev.name || '');
            var formattedType = (typeof formatLabel === 'function') ? formatLabel(dev.type || '') : (dev.type || '-');
            var rearIndicator = (dev.isRear || dev.rear)
                ? '<span class="text-[10px] text-amber-600 font-bold" title="Rear">↩</span>'
                : '<span class="text-[10px] opacity-0">↩</span>';

            html += '<tr class="' + warningClass + ' hover:bg-blue-50 border-b border-slate-100 cursor-pointer' + (disabled ? ' opacity-50' : '') + '" onclick="DeviceDetail.open(' + dev.id + ')">';
            html += '<td class="px-2 py-1.5 text-purple-700 font-semibold truncate" title="' + escapeHtml(locationLabel) + '">📍 ' + escapeHtml(locationLabel) + '</td>';
            html += '<td class="px-2 py-1.5 text-center"><span class="inline-flex items-center justify-center gap-0.5"><span class="px-1.5 py-0.5 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">' + String(dev.order || 0).padStart(2, '0') + '</span>' + rearIndicator + '</span></td>';
            html += '<td class="px-2 py-1.5 font-semibold text-slate-800 truncate" title="' + rawName + '">' + (disabled ? '<span class="text-red-500" title="Disabled">✗</span> ' : '') + (devicePrefix ? '<span class="text-purple-600 font-bold">' + escapeHtml(devicePrefix) + '</span> ' : '') + rawName + '</td>';
            html += '<td class="px-2 py-1.5 text-slate-600 truncate" title="' + escapeHtml(dev.brandModel || '') + '">' + escapeHtml(dev.brandModel || '-') + '</td>';
            html += '<td class="px-2 py-1.5 text-slate-500 truncate">' + formattedType + '</td>';
            html += '<td class="px-2 py-1.5 text-center">' + statusBadge + '</td>';
            html += '<td class="px-2 py-1.5 text-slate-600 truncate" title="' + escapeHtml(addressText) + '">' + (escapeHtml(addressText) || '-') + '</td>';
            html += '<td class="px-2 py-1.5 text-slate-600 truncate" title="' + escapeHtml(dev.service || '') + '">' + escapeHtml(dev.service || '-') + '</td>';
            html += '<td class="px-2 py-1.5 text-center"><span class="text-slate-700">' + (dev.ports ? dev.ports.length : 0) + '</span><span class="text-slate-400">(' + usedPorts + ')</span></td>';
            html += '<td class="px-2 py-1.5 text-center">' + (totalConnections === 0 ? (isWirelessDevice ? '<span class="text-cyan-600 font-semibold">📶</span>' : '<span class="text-orange-600 font-semibold">0⚠</span>') : '<span class="text-slate-700">' + totalConnections + '</span>') + '</td>';
            html += '<td class="px-2 py-1.5 text-center whitespace-nowrap no-print edit-mode-only" onclick="event.stopPropagation()">';
            html += '<a href="#" onclick="DeviceDetail.open(' + dev.id + '); return false;" class="text-blue-500 hover:text-blue-700 text-xs mr-1" title="View Details">🔍</a>';
            html += '<span class="edit-mode-only">';
            html += '<a href="#" onclick="addConnectionFromDevice(' + dev.id + '); return false;" class="text-green-600 hover:text-green-900 text-xs mr-1" title="Add Connection">+Conn</a>';
            html += '<a href="#" onclick="copyDevice(' + dev.id + '); return false;" class="text-purple-600 hover:text-purple-900 text-xs mr-1" title="Duplicate Device">Copy</a>';
            html += '<a href="#" onclick="editDevice(' + dev.id + '); return false;" class="text-blue-600 hover:text-blue-900 text-xs mr-1">Edit</a>';
            html += '<a href="#" onclick="removeDevice(' + dev.id + '); return false;" class="text-red-600 hover:text-red-900 text-xs">Del</a>';
            html += '</span>';
            html += '</td>';
            html += '</tr>';
        }
...
/**
 * Toggle a device group expand/collapse
 */
