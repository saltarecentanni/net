...
var Dashboard = (function() {
    ...
    function renderSearchResultItem(device, query) {
        ...
        html += highlightMatch(getDeviceDisplayName(device) || 'Unnamed', query);
    ...
    function renderConnectionResultItem(conn, idx, query) {
        var fromDevice = getDeviceById(conn.from);
        var toDevice = getDeviceById(conn.to);
        var fromName = fromDevice ? getDeviceDisplayName(fromDevice) : 'Unknown';
        var toName = toDevice ? getDeviceDisplayName(toDevice) : (conn.externalDest || 'External');
