...
var DeviceDetail = (function() {
    ...
    function buildModalContent(device, connections) {
        ...
        html += '<h2 style="margin:0 0 4px 0;font-size:18px;font-weight:700;color:#1e293b;">' + escapeHtml(getDeviceDisplayName(device)) + '</h2>';
        ...
        if (device.isDhcp) {
            html += '<span style="color:#cbd5e1;">•</span>';
            html += '<span style="display:inline-flex;align-items:center;gap:3px;padding:2px 8px;background:#dbeafe;color:#1e40af;border-radius:10px;font-size:10px;font-weight:600;">DHCP</span>';
        }
        // Monitoring badge (v4.0.000) — simple ON/OFF
        if (device.monitoringEnabled) {
            html += '<span style="color:#cbd5e1;">•</span>';
            html += '<span style="display:inline-flex;align-items:center;gap:3px;padding:2px 8px;background:#dcfce7;color:#166534;border-radius:10px;font-size:10px;font-weight:600;">📡 ON</span>';
        }
        html += '</div></div>';
        ...
        // Identity info (v4.0.000) - show only if any field is set
        var hasIdentity = device.serialNumber || device.assetTag || device.macAddress;
        if (hasIdentity) {
            html += '<div style="background:#f8fafc;border-radius:10px;border:1px solid #e2e8f0;padding:12px;margin-bottom:12px;">';
            html += '<div style="font-weight:600;color:#334155;font-size:12px;margin-bottom:8px;">🏷️ Identità</div>';
            html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:11px;">';
            if (device.serialNumber) {
                html += '<div style="color:#64748b;">S/N</div><div style="font-weight:600;font-family:monospace;color:#1e293b;">' + escapeHtml(device.serialNumber) + '</div>';
            }
            if (device.assetTag) {
                html += '<div style="color:#64748b;">Asset Tag</div><div style="font-weight:600;font-family:monospace;color:#1e293b;">' + escapeHtml(device.assetTag) + '</div>';
            }
            if (device.macAddress) {
                html += '<div style="color:#64748b;">MAC</div>';
                html += '<div style="display:flex;align-items:center;gap:4px;">';
                html += '<span style="font-weight:600;font-family:monospace;color:#1e293b;">' + escapeHtml(device.macAddress) + '</span>';
                html += '<button onclick="DeviceDetail.copyToClipboard(\'' + escapeHtml(device.macAddress) + '\')" style="background:none;border:none;cursor:pointer;font-size:10px;color:#94a3b8;padding:0;" title="Copia">📋</button>';
                html += '</div>';
            }
            html += '</div></div>';
        }
        ...
        html += buildNetworkInfo(addresses, device.gateway, device.mask, null, null, device.isDhcp);
        html += '</div>';
    ...
    function buildPortTooltip(port, connections) {
        ...
        if (otherDeviceId) {
            ...
            if (otherDevice) {
                ...
                var otherName = getDeviceDisplayName(otherDevice) || '';
...
// Extrair apenas número (sem sfp, etc)
     ...
     */
    function buildNetworkInfo(addresses, gateway, mask, dns1, dns2, isDhcp) {
        if ((!addresses || addresses.length === 0) && !isDhcp) {
            return '<div style="color:#94a3b8;font-size:11px;text-align:center;">Nessun IP configurato</div>';
        }
        
        var html = '';
        
        // DHCP indicator (v4.0.000)
        if (isDhcp) {
            html += '<div style="display:flex;align-items:center;gap:6px;padding:6px 8px;background:#dbeafe;border-radius:6px;border:1px solid #bfdbfe;margin-bottom:6px;">';
            html += '<span style="font-size:11px;font-weight:600;color:#1e40af;">🔄 DHCP Abilitato</span>';
            html += '</div>';
        }
        ...
        addresses.forEach(function(addr) {
            ...
            // Show IP without /mask suffix when mask is in separate field
            var displayIp = mask ? ipOnly : ip;
            var zone = addr.zone || '';
            ...
            html += '<span style="font-family:monospace;font-size:12px;font-weight:600;color:#1e293b;">' + escapeHtml(displayIp) + '</span>';
        ...
        // DNS info (v4.0.000) - shown regardless of addresses
        if (dns1 || dns2) {
            html += '<div style="font-size:10px;color:#64748b;margin-top:4px;padding-top:4px;border-top:1px solid #e2e8f0;">';
            if (dns1) {
                html += '<div>DNS 1: <span style="font-family:monospace;font-weight:600;">' + escapeHtml(dns1) + '</span></div>';
            }
            if (dns2) {
                html += '<div>DNS 2: <span style="font-family:monospace;font-weight:600;">' + escapeHtml(dns2) + '</span></div>';
            }
            html += '</div>';
        }
    ...
    function buildConnectionsList(device, connections) {
        ...
        connections.forEach(function(conn) {
            ...
            if (otherDeviceId) {
                var otherDevice = findDevice(otherDeviceId);
                otherDeviceName = otherDevice ? getDeviceDisplayName(otherDevice) : 'Device #' + otherDeviceId;
            } else if (conn.externalDest) {
    ...
    function showPortDetail(portName) {
        ...
        if (conn) {
            ...
            if (otherDeviceId) {
                var otherDevice = findDevice(otherDeviceId);
                if (otherDevice) info.push('Connected to: ' + getDeviceDisplayName(otherDevice));
            } else if (conn.externalDest) {
    ...
    function buildHeader(device) {
        return '<div style="display:flex;justify-content:space-between;align-items:center;">' +
               '<h2 class="text-xl font-bold">' + escapeHtml(getDeviceDisplayName(device)) + '</h2>' +
    ...
    function buildContent(device, connections) {
        return buildPortVisualization(device, device.ports || [], connections) +
               ...
               '<div>' + buildNetworkInfo(device.addresses || [], device.gateway, device.mask, null, null, device.isDhcp) + '</div>' +
