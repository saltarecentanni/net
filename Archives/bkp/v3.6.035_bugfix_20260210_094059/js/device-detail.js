...
var DeviceDetail = (function() {
    ...
    function buildModalContent(device, connections) {
        ...
        html += buildNetworkInfo(addresses);
        html += '</div>';
...
// Extrair apenas número (sem sfp, etc)
     ...
     */
    function buildNetworkInfo(addresses) {
        ...
        // Gateway derivado
        if (addresses[0] && addresses[0].network) {
            var gw = addresses[0].network.split('/')[0].split('.');
            if (gw.length === 4) {
                gw[3] = '1';
                html += '<div style="font-size:10px;color:#64748b;margin-top:4px;">Gateway: <span style="font-family:monospace;">' + gw.join('.') + '</span></div>';
            }
        }
    ...
    function buildContent(device, connections) {
        return buildPortVisualization(device, device.ports || [], connections) +
               ...
               '<div>' + buildNetworkInfo(device.addresses || []) + '</div>' +
